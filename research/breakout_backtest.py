"""
Cross-asset backtest for the volatility breakout research strategy.

Runs the Donchian + ATR breakout strategy on crypto (Binance), forex, indices,
and commodities (yfinance). Uses the same OHLCV normalization and pseudo-4h
approach as existing research scripts. All code is research-only; production
...
from research.breakout_strategy import (
...
# -----------------------------------------------------------------------------
# Sanity test mode: if True, only run ETHUSDT; if False, run full cross-asset
# -----------------------------------------------------------------------------
TEST_SINGLE_ASSET = False
...
TRADING_FEE_PERCENT = 0.1  # 0.1% per trade (applied on entry and exit)
...
def normalize_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Same approach as in backtest.py and previous research scripts.
    """
...
def run_single_backtest(
    ...
    """
    Run volatility breakout backtest on one OHLCV series.

    Applies 0.1% cost per trade on entry and exit. Stop loss 2×ATR fixed at entry.
    Position size: 1% risk per trade (risk_amount / stop_distance).
    ...
    required_cols = [
        "previous_20_high", "previous_20_low", "previous_10_high", "previous_10_low",
        "atr_20", "atr_ma", "long_entry", "short_entry", "long_exit", "short_exit",
    ]
...
def run_breakout_backtest(
    ...
    output_csv: str = "breakout_results.csv",
    output_chart: str = "breakout_equity_curves.png",
    results_dir: str = "research_results",
) -> pd.DataFrame:
    """
    Run volatility breakout backtest across configured assets and save results.

    If TEST_SINGLE_ASSET is True, only runs ETHUSDT. Otherwise runs all assets.
    ...
    if TEST_SINGLE_ASSET:
        assets_to_run: List[Tuple[str, Optional[str], str]] = [
            ("ETHUSDT", None, "crypto"),
        ]
    else:
        assets_to_run = []
        for pair in CRYPTO_PAIRS:
            assets_to_run.append((pair, None, "crypto"))
        for display_symbol, yf_ticker in FOREX:
            assets_to_run.append((display_symbol, yf_ticker, "forex"))
        for display_symbol, yf_ticker in INDICES:
            assets_to_run.append((display_symbol, yf_ticker, "indices"))
        for display_symbol, yf_ticker in COMMODITIES:
            assets_to_run.append((display_symbol, yf_ticker, "commodities"))

    for item in assets_to_run:
        display_name = item[0]
        yf_ticker = item[1]
        asset_type = item[2]

        print(f"Running asset: {display_name}")
        try:
            if asset_type == "crypto":
                ohlcv = fetch_binance_klines(
                    display_name, TIMEFRAME, lookback_years=lookback_crypto
                )
            else:
                assert yf_ticker is not None
                if not traditional_lookback_logged:
                    _log.info(
                        "Using lookback period: %s years for traditional assets",
                        lookback_traditional,
                    )
                    traditional_lookback_logged = True
                ohlcv = fetch_yfinance_4h(yf_ticker, lookback_years=lookback_traditional)
            metrics, equity_series, _ = run_single_backtest(ohlcv, display_name)
            row = {"asset": display_name, **metrics}
            results.append(row)
            equity_curves[display_name] = equity_series
    ...
    # Equity curves chart: one normalized curve per asset
    if equity_curves:
        import matplotlib
        matplotlib.use("Agg")
        ...
        ax.set_title(
            "Volatility breakout strategy — equity curves per asset (normalized to 100)"
        )
        ax.set_xlabel("Date")
...
if __name__ == "__main__":
    ...
    run_breakout_backtest(
        ...
        output_chart="breakout_equity_curves.png",
